...
class LivroSerializer(serializers.Serializer):
    ...
    autor = serializers.PrimaryKeyRelatedField(queryset=Autor.objects.all()) # pylint: disable=no-member
    categoria = serializers.PrimaryKeyRelatedField(queryset=Categoria.objects.all()) # pylint: disable=no-member
    publicado_em = serializers.DateField()

    def create(self, validated_data):
        return Livro.objects.create(**validated_data) # pylint: disable=no-member
